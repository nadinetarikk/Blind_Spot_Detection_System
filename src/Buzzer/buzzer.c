...
void buzzer_init2(void)
{
    ...
    // Get the PWM slice corresponding to the GPIO pin
    uint slice_num = pwm_gpio_to_slice_num(BUZZER_PIN2);
...
void buzzer_beep2(uint16_t frequency, uint16_t duration)
{
    ...
    // Set the duty cycle to 50% (2047 out of 4095) for a clean square wave tone
    int half_duty_cycle = 2047;                      // 50% duty cycle of the 12-bit resolution (half of 4095)
    pwm_set_gpio_level(BUZZER_PIN2, half_duty_cycle); // Set the duty cycle to 50%
...
void buzzer_off2(void)
{
    ...
    // Set the duty cycle to 0 to stop the buzzer
    pwm_set_gpio_level(BUZZER_PIN2, 0); // 0 duty cycle (no signal)
}